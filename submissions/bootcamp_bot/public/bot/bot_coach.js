...
function createChallengeSuggestions_v2() {
  var circleNameSuggestions = status.components.scrollView({style: {margin: 10}}, [
    status.components.image(
            {
                source: {uri: "http://mediaprocessor.websimages.com/width/300/crop/0,0,300x155/www.centreks.fr/6572626_orig.jpg"},
                style: {
    ...
    status.components.text({style: {fontWeight: 'bold', textAlign: 'center' }}, 'How to create a challenge'),
    ...
    status.components.text({style: {marginTop: 5, marginLeft: 20}}, 
	 '- Name: "Friday Run", "Marathon training", ...\n' +
 	 '- Reward: Let\s say 5 ETH/week\n' +
	 '- Sport type: Running, Walking, Cycling, Sport Center, ...\n' +
	 '- Localization: Grand Canyon, Central Park, Great Wall of China, ...\n' +
	 '- Time per challenge: 5, 15, 30, 120 minutes ...\n' +
	 '- Time per week: 30, 60, 180 minutes ...'),
...
status.command({
     ...
     params: [{
            ...
            },
            {
             ...
             name: 'location',
             placeholder: 'Where should be the training ?',
             type: status.types.TEXT,
             suggestions: createChallengeSuggestions
           }, {
             name: 'timeChallenge',
             placeholder: 'How long per challenges (minutes) ?',
             type: status.types.NUMBER,
             suggestions: createChallengeSuggestions
           }, {
             name: 'timeWeek',
             placeholder: 'How long per week (minutes) ?',
             type: status.types.NUMBER,
             suggestions: createChallengeSuggestions
           }*/],
      sequentialParams: true,
	...
	if (params.sportType.length < 1) {
      error = status.components.validationMessage(
        'Not a valid sport Type',
        'What is this, your dream ?'
      )
      return {markup: error}
    }
	...
	if (params.timeWeek < 15) {
      error = status.components.validationMessage(
        'Not a valid timeWeek',
        'What is this, your dream ?'
      )
      return {markup: error}
    }
...
/** Check is this is a potential action wished by the user
Input:
	 - messages: String: lower input from the user
	 - list_action: [String]: list of feature that describe an action
	 - ref_action: String: the name of the current action we test
Output:
	Boolean
**/
