...
## Installing and Running the chatbot (abridged version from status.im docs)

* Requires
    Some running mobile device
    Make sure device is connected via network bridging and port 3000/8546 is open
    status-dev-cli is installed (npm install status-dev-cli)
    
    status-dev-cli scan (gets you an IP of your device and this becommes your DEVICE-IP)
    status-dev-cli list --ip <DEVICE-IP>
    status-dev-cli add --botUrl "http://localhost:3000/bot/bot.js" --ip DEVICE-IP (adds your bot to the device and you should now see it!)
    status-dev-cli list --ip <DEVICE-IP> (you'll see the bot now listed)
    status-dev-cli remove BOT_IDENTITy --ip DEVICE-IP (removes the bot)
