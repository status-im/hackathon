...
class EditBet extends Component {
  ...
  render() {
    ...
    const EditBetGUI = () => (
      <div>
        <h3>Your possiton is {betForAgainst? 'FOR' : 'AGAINST'}</h3>
        <Toggle
          label={betForAgainst? 'FOR' : 'AGAINST'}
        ...
        <RaisedButton onTouchTap={EditBetButton} label='Set bet position' primary={true} fullWidth={true}/>
      </div>
    )
    return (
