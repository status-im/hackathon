...
export class CourseAddPage {
    ...
    submitCourse() {
        ...
        let successNotif = this.alertCtrl.create({
          ...
          subTitle: 'Your course has been submitted.',
          buttons: [{
              text: 'OK',
              handler: data => {
        ...
        this.contractService.registerCourse(this.newCourseForm.value.studentAddress, startDate.getTime(), endDate.getTime(), this.newCourseForm.value.amount, this.newCourseForm.value.description, this.newCourseForm.value.level).then(function(res) {
